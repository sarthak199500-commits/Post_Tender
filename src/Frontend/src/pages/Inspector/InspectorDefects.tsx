    ...
    <div className="space-y-8">
      <div className="flex justify-between items-start">
        ...
        <button 
          ...
          className="bg-brand-600 text-white px-6 py-3 rounded-card font-bold flex items-center gap-2 hover:bg-brand-700 transition-colors shadow-lg shadow-brand-200"
        >
      ...
      <div className="space-y-6">
        {inspections.length === 0 ? (
            <div className="bg-white p-12 text-center rounded-card border border-slate-200 shadow-sm">
                                ...
                                <div className="flex-1">
                                    <div className="flex items-center gap-2 mb-2">
                                        ...
                                        <span className={`text-[10px] font-black uppercase tracking-widest px-2 py-0.5 rounded ${
                                            ...
                                            defect.status === 'Rectified' ? 'bg-brand-100 text-brand-700 border border-brand-200' : 'bg-emerald-100 text-emerald-700'
                                        }`}>
                                            {defect.status}
                                        </span>
                                    </div>
                                    ...
                                    {defect.reworkReportUrl && (
                                        <div className="mt-3">
                                            <a href={`${GATEWAY_BASE}${defect.reworkReportUrl}`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 text-xs font-bold text-brand-600 bg-brand-50 px-3 py-1.5 rounded-control border border-brand-100 hover:bg-brand-100 transition-colors">
                                ...
                                {defect.status === 'Rectified' && (
                                    <div className="flex gap-2">
                                        <button 
                                            ...
                                            className="bg-emerald-700 text-white px-4 py-2 rounded-control text-sm font-bold hover:bg-emerald-800 transition-colors shadow-sm"
                                        >
                                            Accept & Close
                                        </button>
                                        <button 
                                            onClick={() => handleVerifyDefect(defect.id, false)}
                                            className="bg-white text-red-600 border border-red-200 px-4 py-2 rounded-control text-sm font-bold hover:bg-red-50 transition-colors"
                                        >
                                            Reject Rework
                                        </button>
                                    </div>
            ...
            <div className="bg-white rounded-card shadow-2xl w-full max-w-2xl overflow-hidden border border-slate-200">
               ...
               <div className="p-6 space-y-6 max-h-[60vh] overflow-y-auto">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                          ...
                          <select aria-label="Select an option" 
                              ...
                              className="w-full p-3 border border-slate-200 rounded-card focus:ring-2 focus:ring-brand-500 outline-none font-medium bg-slate-50"
                          >
                  ...
                  <div>
                      ...
                      <label className={`block border-2 border-dashed rounded-card p-6 text-center transition-colors cursor-pointer ${
                  ...
                  <div className="border border-slate-200 rounded-control p-4 bg-slate-50">
                      ...
                      <div className="flex gap-4">
                          ...
                          <select aria-label="Select an option" 
                              ...
                              className="w-32 p-3 border border-slate-200 rounded-card focus:ring-2 focus:ring-brand-500 outline-none text-sm font-medium"
                          >
                              {DEFECT_SEVERITIES.map(s => <option key={s} value={s}>{s}</option>)}
                          </select>
                          <button 
                              onClick={handleAddDefect}
                              className="bg-slate-800 text-white px-4 py-2 rounded-card font-bold hover:bg-slate-700 transition-colors"
                          >
                              Add
                          </button>
                      </div>
                  ...
                  {newInspection.defects.length > 0 && (
                      <div className="space-y-2">
                          ...
                          {newInspection.defects.map((d, i) => (
                              <div key={i} className="flex items-center justify-between p-3 bg-white border border-slate-200 rounded-card shadow-sm">
                                  ...
                                  <button onClick={() => handleRemoveDefect(i)} className="text-red-700 hover:bg-red-50 p-1.5 rounded-control"><X className="w-4 h-4" /></button>
                              </div>
                          ))}
                      </div>
                  )}
               ...
               <div className="p-6 bg-slate-50 border-t border-slate-100 flex justify-end gap-3">
                  <button onClick={() => setIsLogging(false)} className="px-6 py-2.5 rounded-card font-bold text-slate-600 hover:bg-slate-100">Cancel</button>
                  <button 
                    onClick={handleSubmitInspection}
                    className="bg-brand-600 text-white px-8 py-2.5 rounded-card font-bold hover:bg-brand-700 transition-colors shadow-lg shadow-brand-200"
                  >
                    Submit Inspection
                  </button>
               </div>
