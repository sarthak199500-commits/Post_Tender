    ...
    return (
        <div>
            <header className="mb-10 flex justify-between items-start">
                <div>
                    <Link to="/inspector/work-orders" className="text-brand-700 font-bold text-sm hover:underline mb-4 inline-flex items-center gap-1">&larr; Back to List</Link>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="bg-white p-8 rounded-card shadow-sm border border-slate-100">
                        ...
                        <div className="space-y-4">
                            {wo.milestones.map((m, idx) => (
                                <div key={m.id} className="flex items-center gap-6 p-4 bg-slate-50 rounded-control relative">
                                    ...
                                    {m.status === 'Inspection Requested' && (
                                        ...
                                        <Link
                                            ...
                                            className="bg-brand-50 hover:bg-brand-100 text-brand-700 border border-brand-200 text-xs font-bold px-3 py-1.5 rounded-control transition-colors ml-4 shadow-sm inline-block"
                                        >
                                            View Submission
                                        </Link>
                                    )}
                                    {m.status === 'Completed' && (
                                        <Link
                                            to={`/inspector/milestones/${m.id}/submission`}
                                            className="bg-slate-100 hover:bg-slate-200 text-slate-600 border border-slate-200 text-xs font-bold px-3 py-1.5 rounded-control transition-colors ml-4 shadow-sm inline-block"
                                        >
                                            View Submission
                                        </Link>
                                    )}
                ...
                <div className="space-y-8">
                    ...
                    <div className="bg-slate-900 p-8 rounded-card text-white shadow-xl">
                        ...
                        <div className="text-brand-400 text-xs font-bold mb-6">{wo.vendor.vendorCode}</div>
                    ...
                    <div className="bg-white p-6 rounded-card border border-slate-100 shadow-sm">
                        ...
                        <div className="space-y-3">
                            <Link to={`/inspector/visits/schedule?woId=${wo.id}`} className="block w-full bg-brand-600 text-white text-center py-3 rounded-card font-bold text-sm hover:bg-brand-700 transition-colors">
                                Schedule Inspection
                            </Link>
                            {/* There is no /inspector/observations route — recording an
                                observation is what the Quality Defects page does. */}
                            <Link to="/inspector/defects" className="block w-full bg-white text-slate-700 border border-slate-200 text-center py-3 rounded-card font-bold text-sm hover:bg-slate-50 transition-colors">
                                Record Observation
                            </Link>
                        </div>
                    </div>
